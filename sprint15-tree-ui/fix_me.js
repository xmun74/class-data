...
function createTreeView(menu, currentNode) {
  // 제일 겉에 menu 순회하면서 li 생성
  for (let i = 0; i < menu.length; i++) {
    const li = document.createElement("li");

    // 자식이 있으면 input, span, ul 생성 => li에 자식으로 넣기 => 재귀로 반복
    if (menu[i].type === "group") {
      ...
      li.append(input, span, ul); // li에 자식으로 넣기

      createTreeView(menu[i].children, ul); // 재귀로 반복
    }
    // 자식 없으면 name만 넣기
    else {
      li.textContent = menu[i].name;
    }
    currentNode.append(li); // 조건 다 넣으면 ul에 만든 li 넣기
  }
}
